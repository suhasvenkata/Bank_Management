...
int generateAccountNumber() {
    // Using timestamp as a part of the account number to ensure uniqueness
    time_t now = time(nullptr);
    int timestamp = static_cast<int>(now);

    // Combine timestamp with a random number to form a new account number
    int newAccountNumber = (timestamp % 1000000) + (rand() % 1000); // You can adjust the range as needed
